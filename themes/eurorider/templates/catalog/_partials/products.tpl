...
<div id="js-product-list">
  ...
  {if $cars_products_page}
    {if $compat}
      <article id="current_car_settings" class=" js-product-miniature d-flex justify-content-center col-xl-3 col-lg-4 col-md-4  col-sm-6 col-xs-12" itemscope itemtype="http://schema.org/Product" style="background: #efefef !important;display:flex;flex-direction: column;border-radius:0.25rem;border: 2px solid var(--asm-color);" id_compat="{$compat['id_compat']}">
      <div style="display:flex;flex-direction:column;align-items:center;height:100%;border-radius:.25rem;padding:1rem;width:100%;justify-content:center;">
          ...
          <div class="current-car-content">
            ...
            <div class="mobile details-info-car-mobile">
              <span><img src="{$compat['brand_hover_logo']}" style="width: 40px;"/></span>
              <span>|</span>
              <span style="color: var(--asm-color) !important;">{$compat['model']}</span>
              <span>|</span>
              <span>{$compat['type']}</span>
              <br>
              <span>{$compat['version']}</span>
            </div>
