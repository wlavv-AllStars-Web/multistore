    ...
    if($(this).val())
    {
        ...
        $.ajax({
            ...
            success: function(json){
                ...
                $('.block-onepagecheckout').removeClass('loading');
                // $(".block-shipping .delivery-options .delivery-option input[type='radio'][value='6,']").click();
            },
            error: function(error)
            {
                $('.block-onepagecheckout').removeClass('loading');
...
$(document).on('click','button[name="submitCompleteMyOrder"]',function(e){ 
    ...
    // const formPayment = document.querySelector(`#pay-with-payment-option-`+globalIdForm+`-form form`)
    // console.log('deu')
    // formPayment.submit()
    ...
    $.ajax({
        ...
        success: function(json){ 
            ...
            {
                // console.log($('input[name="payment-option"]:checked'))
                if($('input[name="payment-option"]:checked').length)
                {
        ...
        $.ajax({
            ...
            success: function(json){ 
                ...
                if(json.cart_detailed=='empty')
                {
                    ...
                    $('#form_ets_onepagecheckout').html(`<div class="cart_empty" style="display:flex;flex-direction:column;justify-content:center;align-items: center;"><img style="width:100%;" src="/img/asd/Content_pages/shopping_cart/shopping_${language_iso}.webp" /> <div style="padding:2rem 0;width:100%;display:flex;flex-direction:column;justify-content:center;align-items: center;gap:1rem;"><div class="alert alert-warning" style="width:100%;max-width:1350px;">${json.text_info}</div><a class="btn btn-primary" href="${json.link}">${json.text_btn}</a></div></div>`);
                    $(".cart-container .productsValue").replaceWith("0.00 €"); 
                    $(".cart-container .products_total_header div").text("0"); 
...
function ets_opc_displayShipping()
{
    $.ajax({
        url: $('#form_ets_onepagecheckout').attr('action'),
        data: 'updateCarrier=1'+($('.delivery-options-list :input').length ? '&'+$('.delivery-options-list :input').serialize():'')+'&id_country='+$('.shipping_address .ets-onepage-js-country').val()+'&type_checkout_options='+($('input[name="type-checkout-options"]:checked').length ? $('input[name="type-checkout-options"]:checked').val():'login')+($('#shipping_address_id_state').length ? '&id_state='+$('#shipping_address_id_state').val() :'')+($('#shipping_address_postal_code').length ? '&postal_code='+$('#shipping_address_postal_code').val():'')+'&delivery_option[10]=6',
        type: 'post',
        dataType: 'json',
        success: function(json){
            if(json.total_more_free)
