      ...
      <article id="store-{$store.id}" class="store-item card">
        <div class="store-item-container clearfix">
          ...
          <div class="col-md-5 col-sm-7 col-xs-12 store-description">
            ...
            {if $store.note || $store.phone || $store.fax || $store.email}
              <a data-toggle="collapse" href="#about-{$store.id}" aria-expanded="false" aria-controls="about-{$store.id}"><strong>{l s='About and Contact' d='Shop.Theme.Global'}</strong><i class="material-icons" translate="no">&#xE409;</i></a>
            {/if}
          </div>
        ...
        <footer id="about-{$store.id}" class="collapse">
          <div class="store-item-footer divide-top">
            ...
            <ul class="card-block">
              {if $store.phone}
                <li><i class="material-icons" translate="no">&#xE0B0;</i>{$store.phone}</li>
              {/if}
              {if $store.fax}
                <li><i class="material-icons" translate="no">&#xE8AD;</i>{$store.fax}</li>
              {/if}
              {if $store.email}
                <li><i class="material-icons" translate="no">&#xE0BE;</i>{$store.email}</li>
              {/if}
            </ul>
          </div>
        </footer>
