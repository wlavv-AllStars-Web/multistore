...
class LanguageCore extends ObjectModel implements LanguageInterface
{
    ...
    public static function loadLanguages()
    {
        ...
        usort($result, function ($a, $b) use ($order) {
            $pos_a = array_search((int) $a['id_lang'], $order);
            $pos_b = array_search((int) $b['id_lang'], $order);

            // Ensure elements not in the order array are placed at the end
            $pos_a = $pos_a !== false ? $pos_a : count($order);
            $pos_b = $pos_b !== false ? $pos_b : count($order);
